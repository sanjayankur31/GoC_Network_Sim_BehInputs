...
def GJ_conn(
    ...
    dpairs = [dist_2D[GJ_pairs[j, 0], GJ_pairs[j, 1]] for
              j in range(GJ_pairs.shape[0])]
...
def connect_inputs(
    ...
    if connType == 'random_prob':
        ...
        conn_pairs = set_random_inputs(nInp, nGoC, pConn=connProb,
                                       nConn=connGoC)
    ...
    if connDist[0] > 0:
        ...
        if len(connDist) == 3:
            ...
            conn_pairs = conn_pairs[
                :,
                [(
                 (abs(Inp_pos[conn_pairs[0, jj], 0] -
                  GoC_pos[conn_pairs[1, jj], 0]) < connDist[0]) &
                 (abs(Inp_pos[conn_pairs[0, jj], 1] -
                  GoC_pos[conn_pairs[1, jj], 1]) < connDist[1]) &
                 (abs(Inp_pos[conn_pairs[0, jj], 2] -
                  GoC_pos[conn_pairs[1, jj], 2]) < connDist[2]))
            ...
            conn_pairs = conn_pairs[
                :,
                [(
                 np.power(Inp_pos[conn_pairs[0, jj], 0] -
                          GoC_pos[conn_pairs[1, jj], 0], 2) +
                 np.power(Inp_pos[conn_pairs[0, jj], 1] -
                          GoC_pos[conn_pairs[1, jj], 1], 2) +
                 np.power(Inp_pos[conn_pairs[0, jj], 2] -
                          GoC_pos[conn_pairs[1, jj], 2], 2)) < connDist[0]**2
                 for jj in range(conn_pairs.shape[1])]
             ]
...
def PF_conn(  # NOT USED
    ...
    if PF_conndist[0] > 0:
        ...
        PF_pairs = PF_pairs[
            :,
            [(
             (abs(PF_pos[PF_pairs[0, jj], 0] -
              GoC_pos[PF_pairs[1, jj], 0]) < PF_conndist[0]) &
             (abs(PF_pos[PF_pairs[0, jj], 1] -
              GoC_pos[PF_pairs[1, jj], 1]) < PF_conndist[1]) &
             (abs(PF_pos[PF_pairs[0, jj], 2] -
              GoC_pos[PF_pairs[1, jj], 2]) < PF_conndist[2]))
