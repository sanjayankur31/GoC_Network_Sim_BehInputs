...
def get_simulation_params(
    ...
    for input_ in behInputs:
        ...
        Inp["filename"] = Input_flnm[input_]
    ...
    idctr = 7
    for input_ in clusInputs:
        idctr += 1
        spktype = clusType[input_]
        Inp = {
            "type": spktype,
        ...
        Input_id[input_] = idctr
        Inp["nInp"] = int(nInputs_max[spktype] * nInputs_frac[spktype])

        # restrict inputs to subvolume
        Inp["nInp"], Inp["pos"] = net_utils.locate_GoC(
            nGoC=Inp["nInp"],
            volume=clusVolume[input_],
            seed=simid * ctr + Input_id[input_],
        )
        Inp["pos"] = Inp["pos"] + clusSpatialOffset[input_]

        Inp["filename"] = Input_flnm[input_]

        tmp = {"conn_pairs": [], "conn_wt": [], "conn_loc": []}
        currseed = (
            simid * Inp["nInp"] * ctr + Input_id[input_] + clusSpatialOffset[input_][1]
        )
        if Inp["nInp"] > 0:
            np.random.seed(currseed)
            Inp["sample"] = np.random.permutation(input_nFiles[spktype])[
                0 : Inp["nInp"]
            ]  # choose wh
