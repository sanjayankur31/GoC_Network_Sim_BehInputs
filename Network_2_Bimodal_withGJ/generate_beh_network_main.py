...
def create_GoC_network(
    ...
    ### ---------- Load Params
    p = get_params(runid=runid, mf=mf, pf=pf, mf2=mf2, pf2=pf2, hom=hom)

    hom_str = "hom" if hom else "het"
    # Build network to specify cells and connectivity
    datadir = "../Data_BiNet_lessBG_" + hom_str + "/"
...
def get_params(runid=0, mf=0.2, pf=0.2, mf2=0.1, pf2=0.1, hom=False):
    Input_prob = {
        "MF_bg": 0.3,
        "PF_bg": 0.5,
        "MFON": 0.2,
