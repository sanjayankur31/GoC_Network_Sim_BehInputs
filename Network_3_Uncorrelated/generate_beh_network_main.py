...
def create_GoC_network(
    ...
    mf2/pf2 = randomly permuted mf/pf inputs (fraction of max)
    """

    ### ---------- Load Params
    p = get_params(runid=runid, mf=mf, pf=pf, mf2=mf2, pf2=pf2)

    gje_str = "wGJ" if gje else "noGJ"
    # Build network to specify cells and connectivity
    datadir = "../Data_RandNet_" + gje_str + "/"
    ...
    simid = (
        ...
        + "_mf2_"
        + format(int(mf2 * 100), "03d")
        + "_pf2_"
        + format(int(pf2 * 100), "03d")
        + "_"
        + gje_str
    )
    ...
    arr_flnm = [
        ...
        "../Mechanisms/SpikeArray_MFOFF_dur_060_080.nml",
        "../Mechanisms/SpikeArray_RandomWin_MF_dur_020.nml",
        "../Mechanisms/SpikeArray_RandomWin_PF_dur_020.nml",
    ]
...
def get_params(runid=0, mf=0.2, pf=0.2, mf2=0.1, pf2=0.1):
    Input_prob = {
        "MF_bg": 0.3,
        "PF_bg": 0.5,
        "MFON": 0.2,
    ...
    params = inp.get_simulation_params(
        ...
        stepInputs=[],
        behInputs=["MFON", "PFON", "MFRand", "PFRand"],
    )
    return params
