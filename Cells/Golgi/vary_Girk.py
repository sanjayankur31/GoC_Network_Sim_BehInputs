...
def create_GoC(runid=0, usefile="cellparams_file.pkl", girk=0):
    ...
    gocID = "GoC_" + format(runid, "05d") + "_girk_" + format(int(1e3 * girk))
    ...
    chan_girk = nml.ChannelDensity(
        ion_channel=pynml.read_neuroml2_file(girk_fname).ion_channel[0].id,
        cond_density="{} mS_per_cm2".format(girk),
        erev="-84.69 mV",
        ion="k",
